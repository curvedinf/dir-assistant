...
if dir_assistant_active_model_is_local:
    ...
    dir_assistant_litellm_model = input(
        "Enter the LiteLLM identifier of the model (https://docs.litellm.ai/docs/providers) or press enter for gemini-1.5-flash-latest: "
    )
    if dir_assistant_litellm_model == "":
        dir_assistant_litellm_model = "gemini/gemini-1.5-flash-latest"
    dir_assistant_litellm_context_size = input("Enter the context size for your API model in tokens or press enter for 500000: ")
    ...
    dir_assistant_litellm_pass_through_context_size = input("Does your API need the context size passed through? (most do not) (y/n): ").lower() == "y"
    local_llm_on_gpu = False
embedding_model_on_gpu = input("Would you like your embedding model to run on your GPU? (y/n): ").lower() == "y"
...
if embedding_model_on_gpu:
    dir_assistant_llama_cpp_embed_options["n_gpu_layers"] = -1
...
config = {
    ...
    "DIR_ASSISTANT_LITELLM_PASS_THROUGH_CONTEXT_SIZE": dir_assistant_litellm_pass_through_context_size,
