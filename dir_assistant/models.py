...
def models_download_embed(args, config_dict):
    model_path = get_file_path(config_dict['DIR_ASSISTANT']['MODELS_PATH'], MODELS_DEFAULT_EMBED)
    run([
        'wget',
        '-O',
        model_path,
        MODELS_DEFAULT_EMBED_URL
    ])
    config_dict['DIR_ASSISTANT']['EMBED_MODEL'] = MODELS_DEFAULT_EMBED
    save_config(config_dict)

def models_download_llm(args, config_dict):
    model_path = get_file_path(config_dict['DIR_ASSISTANT']['MODELS_PATH'], MODELS_DEFAULT_LLM)
    run([
        'wget',
        '-O',
        model_path,
        MODELS_DEFAULT_LLM_URL
    ])
