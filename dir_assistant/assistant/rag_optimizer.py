...
class RagOptimizer:
    ...
    def optimize_rag_for_caching(
        ...
        for p in permutations(top_n_artifacts):
            ...
            is_in_active_cache = (
                1 if current_prefix_string in prefix_cache_metadata else 0
            )
            current_prefix_score = (
                historical_hits * self.weights.get("historical_hits", 1.0)
            ...
            if current_prefix_score > max_prefix_match_score:
                ...
                remaining_artifacts = [
                    art for art in initial_sorted_artifacts if art not in perm
                ]
